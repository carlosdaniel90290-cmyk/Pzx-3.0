...
local function KillMobsInIsland(islandPos)
    ...
    -- 1. Coleta e filtra todos os monstros válidos dentro do raio da ilha
    for _, mob in pairs(enemies:GetChildren()) do
        local hum = mob:FindFirstChildOfClass("Humanoid")
        local root = mob:FindFirstChild("HumanoidRootPart")
        
        if hum and root and hum.Health > 0.1 then
            local dist = (root.Position - islandPos).Magnitude
            if dist <= ISLAND_RADIUS + 100 then
    ...
    -- Se não houver mobs na área, retorna falso
    if #activeMobs == 0 then return false end

    didHit = true

    -- Posiciona no centro/perto do primeiro mob uma única vez para estabilizar o raio
    if activeMobs[1] and activeMobs[1].root then
        TP(activeMobs[1].root.CFrame * CFrame.new(0, 5, -7))
    end

    -- 2. Sistema de ataque simultâneo (Paralelo via task.spawn)
    for _, data in ipairs(activeMobs) do
        task.spawn(function()
            -- Cada monstro tem sua própria thread rodando ao mesmo tempo que os outros
            while _G.Raiding and data.mob and data.mob.Parent and data.hum and data.hum.Health > 0.1 and InRaid() do
                if (data.root.Position - islandPos).Magnitude > ISLAND_RADIUS + 120 then 
                    break 
                end

                if Attack and Attack.Kill then
                    pcall(function()
                        -- Dispara o ataque diretamente no alvo de forma isolada e instantânea
                        Attack.Kill(data.mob, true)
                    end)
                end

                -- Intervalo curtíssimo para manter alta performance sem crashar o executor
                task.wait(0.02)
            end
        end)
    end
...
local Window = redzlib:MakeWindow({
  Title = "KZ MENU PREMIUM SCRIPT",
  SubTitle = "BY @LEO MODZ",
  SaveFolder = "LeoMenuHub.lua"
})

Window:AddMinimizeButton({
    Button = { Image = "rbxassetid://12940626152", BackgroundTransparency = 0 },
...
Tabs.Info:AddSection("Information")
    ...
    Tabs.Info:AddButton({
    Title = "TELEGRAM @LEOMODZOFC",
    Description = "Contact Me!!!",
    Logo = "rbxassetid://12940626152",
    Invite = "@LEOMODZOFC",
})
...
AttackDropdown = Tabs.Main:AddDropdown({
    ...
    Options = {"Normal Attack","Fast Attack","Super Fast Attack","Kz Menu Attack"},
    Default = "Fast Attack",
    Callback = function(Value)
...
Tabs.Main:AddSection({"Collect Money"})
